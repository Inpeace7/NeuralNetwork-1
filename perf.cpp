...
void Network::FeedFwd(const Value& InVals) {
	...
	for(int i = 0; i < InVals.size(); ++i){
        ...
        //std::cout << "2==>" << InVals.size() << std::endl;
        //std::cout << "3==>" << InVals[i] << std::endl;
...
int BigPrNum = 1e3;
int PrNum = 1e5;
int tt(std::vector<Value>& prvec){
    int result = 0;
    for (int d = 0; d < BigPrNum; d++) {
        for (int k = 0; k < PrNum; k++){
            result = prvec[k][0] / prvec[k][1];
            //std::cout << prvec[k][0] << " " << prvec[k][1] << " "<< result << std::endl;
        }
    }
...
int main()
{
    ...
    std::cout << dwEnd.tv_sec << " " << dwStart.tv_sec << " " << dwEnd.tv_usec << " " << dwStart.tv_usec << std::endl;

    int interval = 1000000*(dwEnd.tv_sec-dwStart.tv_sec)+(dwEnd.tv_usec-dwStart.tv_usec);
    std::cout << interval << std::endl;
    std::cout << double(interval)/1000000 << std::endl;
    std::cout << (PrNum/double(interval))*1000000*BigPrNum << std::endl;
